...
Move your lines of code for finding out the time now, extracting the hour, minutes and seconds to variables, and displaying the time, so these lines are 

    1) *below* the ```while True:``` line and

    2) **INDENTED**  to show they are part of the loop.
